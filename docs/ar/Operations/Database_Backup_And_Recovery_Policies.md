...
يُحدّد هذا الدليل سياسات وإجراءات النسخ الاحتياطي لقاعدة البيانات، والاستعادة في لحظة زمنية محددة (PITR)، وإدارة إصدارات المخطط في ACCSYSTEM ERP. يُوجَّه إلى مسؤولي قواعد البيانات (DBAs) ومهندسي DevOps المسؤولين عن استمرارية البيانات. نظراً لأن ACCSYSTEM ERP يُطبّق الثبات المالي (Financial Immutability) في General Ledger (دفتر الأستاذ العام)، فإن سلامة المستودع الدائم تمثّل اهتماماً مؤسسياً بالغ الأهمية.

## النطاق والتطبيق

تشمل هذه السياسة قاعدة بيانات PostgreSQL الرئيسية لـ ACCSYSTEM ERP، بما في ذلك جميع المخططات التي أنشأتها 142 ملف migration (هجرة) مُدار في `backend/database/migrations/`. تشمل جميع البيئات: الإنتاج والتجريبية والتطوير.
