...
The first release is **loopback-only**. MariaDB binds exclusively to `127.0.0.1:3307`; the Laravel API binds exclusively to `127.0.0.1:8765`; and lifecycle control uses the Windows Service Control Manager through an elevated Agent invocation. The installer creates machine-generated application, database, and database-root credentials, removes the default anonymous and test-database surfaces, and records secrets only in an ACL-restricted data root. Neither is included in release assets, manifests, logs, diagnostics, screenshots, or the content-addressable runtime cache.
...
The release gate validates that database and API listeners reject non-loopback binding, that no local control TCP listener or ordinary-user lifecycle path is present, that the private data root rejects ordinary-user writes, and that diagnostics redact secret values.
