...
Route::group(['prefix' => 'platform', 'middleware' => 'can:settings,view'], function () {
    ...
    Route::group(['prefix' => 'compliance'], function() {
        ...
        Route::post('/profiles/{id}/revoke-token', [ComplianceProfileController::class, 'revokeToken'])->name('v2.platform.compliance.token.revoke.legacy'); // POST Alias
        ...
        Route::post('/validate-structure', [ComplianceProfileController::class, 'validateStructure'])->name('v2.platform.compliance.validate.legacy'); // Alias
    });
});

// ── Public Compliance External Pull (No Auth - Uses Bearer/System Tokens)
Route::group(['prefix' => 'platform/compliance', 'middleware' => ['throttle:api-sensitive']], function () {
    Route::post('/pull-data', [ComplianceProfileController::class, 'servePullData'])->name('v2.platform.compliance.pull');
    
    // Legacy compatibility
    Route::get('/pull/{code}/{path?}', [ComplianceProfileController::class, 'servePullData'])->name('v2.platform.compliance.pull.legacy');
    Route::get('/compliance-pull/{code}/{path?}', [ComplianceProfileController::class, 'servePullData'])->name('v2.platform.compliance_pull.legacy');
});
