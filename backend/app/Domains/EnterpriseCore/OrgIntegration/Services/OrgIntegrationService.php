...
namespace App\Domains\EnterpriseCore\OrgIntegration\Services;
...
class OrgIntegrationService
{
    ...
    /**
     * Bulk sync multiple targets.
     */
    public function bulkSync(array $targets): array
    {
        $results = [];
        foreach ($targets as $target) {
            $results[$target] = match ($target) {
                'cost_centers'   => $this->syncAllCostCentersToOrgChart(),
                'profit_centers' => $this->syncAllProfitCentersToOrgChart(),
                'job_titles'     => $this->syncAllJobTitles(),
                'nodes_to_tables' => $this->syncAllOrgNodesToTables(),
                default          => ['error' => 'Unknown target'],
            };
        }
        return $results;
    }
