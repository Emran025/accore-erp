...
export default function ViewCustomersGroupsPage() {
    const router = useRouter();
    const { objectData, isLoading, createObject, saveGroup, deleteGroup } = useNumberRange({ objectType: "ar_customers" });
    ...
    return (
        <MainLayout>
            <PageSubHeader
                ...
                titleIcon="layers"
                actions={
                    <Button variant="primary" icon="plus" onClick={() => router.push("/ar_customers/groups-number-range-interval/ar_customers/add-ar_customers-group")}>
                        إضافة تجميع جديد
                    </Button>
                }
            />
