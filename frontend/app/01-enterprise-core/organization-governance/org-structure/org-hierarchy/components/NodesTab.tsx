...
export function NodesTab({ isSetupFlow = false }: NodesTabProps) {
  ...
  const loadNodes = useCallback(async () => {
    try {
      ...
      showToast(i18n.catalog['enterpriseCore.nodes.errorLoadingOrganizationalUnits'], 'error');
    } finally {
      setIsLoading(false);
    }
  ...
  const loadMetaTypes = useCallback(async () => {
    try {
      ...
      showToast(i18n.catalog['common.general.errorLoadingUnitTypes'], 'error');
    }
  }, []);
  ...
  useEffect(() => {
    ...
    loadMetaTypes();
    loadTopologyRules();
  }, [loadNodes, loadMetaTypes, loadTopologyRules]);

  const getTypeLabel = (id: string) =>
    metaTypes.find((t) => t.id === id)?.display_name_ar ||
    metaTypes.find((t) => t.id === id)?.display_name ||
    id;
  const getTypeDomain = (id: string) => metaTypes.find((t) => t.id === id)?.level_domain || '';

  const filteredMetaTypes = filterDomain
    ? metaTypes.filter((t) => t.level_domain === filterDomain)
    : metaTypes;
  ...
  const openEditDialog = (node: StructureNode) => {
    ...
    Object.entries(attrs).forEach(([k, v]) => {
      attrMap[k] = typeof v === 'object' ? JSON.stringify(v) : String(v || '');
    });
    setFormData({
  ...
  const handleSubmit = async () => {
    if (!formData.node_type_id.trim() || !formData.code.trim()) {
      showToast(i18n.catalog['enterpriseCore.nodes.pleaseEnterTypeCode'], 'error');
      return;
    }
    ...
    if (formData.target_node_uuid && !selectedNode) {
      payload.link = {
        target_node_uuid: formData.target_node_uuid,
        validate_constraints: formData.validate_constraints,
      };
    }
    try {
      if (selectedNode) {
        ...
        await fetchAPI(API_ENDPOINTS.ENTERPRISE_CORE.ORG.NODES, {
          method: 'POST',
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showToast(i18n.catalog['enterpriseCore.nodes.unitDeleted'], 'success');
      loadNodes();
    } catch (e: unknown) {
      const err = e as { message?: string };
  ...
  const handleBulkStatus = async () => {
    ...
    try {
      ...
      showToast(
        catalogText(i18n, 'enterpriseCore.nodes.moduleUpdated', { value0: selectedUuids.size }),
        'success'
      );
      setSelectedUuids(new Set());
      setBulkDialog(false);
      ...
      showToast(
        (e as { message?: string })?.message ||
          i18n.catalog['enterpriseCore.nodes.bulkUpdateError'],
        'error'
      );
    }
  };
  ...
  const nodeColumns: Column<StructureNode>[] = [
    ...
    },
    {
      ...
      dataLabel: i18n.catalog['common.general.code'],
    },
    {
      key: 'node_type_id',
      ...
      render: (item) => {
        ...
        return (
          ...
          >
            {getTypeLabel(item.node_type_id)}
          </span>
        );
      },
    },
    ...
    {
      key: 'links',
      header: i18n.catalog['common.general.links.alternative2'],
      dataLabel: i18n.catalog['common.general.links.alternative2'],
      render: (item) => {
        ...
        return (
          <div style={{ display: 'flex', gap: '4px', fontSize: '0.75rem' }}>
            <span title={i18n.catalog['enterpriseCore.nodes.issued']} style={{ color: '#3b82f6' }}>
              ↑{out}
            </span>
            <span
              title={i18n.catalog['enterpriseCore.nodes.incoming']}
              style={{ color: '#10b981' }}
            >
              ↓{inc}
            </span>
    ...
    {
      key: 'validity',
      header: i18n.catalog['common.general.permission'],
      dataLabel: i18n.catalog['common.general.permission'],
      render: (item) => {
    ...
    },
    {
      ...
      render: (item) => (
        <span
          className={`badge ${item.status === 'active' ? 'badge-success' : item.status === 'inactive' ? 'badge-warning' : 'badge-secondary'}`}
        >
          {item.status === 'active'
            ? i18n.catalog['common.general.active']
            : item.status === 'inactive'
              ? i18n.catalog['common.general.inactive']
              : i18n.catalog['common.general.archived']}
        </span>
      ),
    },
    {
        ...
        <ActionButtons
          actions={[
            ...
            },
            {
              ...
              variant: 'delete',
              onClick: () => confirmDelete(item.node_uuid),
            },
          ]}
        />
      ),
  ...
  const showInitialSetup =
    isSetupFlow && initialDataLoaded && !isLoading && nodes.length === 0 && metaTypes.length > 0;
  const setupNodeColumns: Column<StructureNode>[] = [
    ...nodeColumns.filter((column) => ['code', 'node_type_id', 'status'].includes(column.key)),
    {
      key: 'actions',
      header: i18n.catalog['common.general.actions'],
      dataLabel: i18n.catalog['common.general.actions'],
  ...
  if (showInitialSetup && !formDialog) {
    return (
      <>
        <InitialNodeSetup {...formProps} onSubmit={handleSubmit} isSubmitting={isSubmitting} />
      </>
    );
  }
      ...
      <>
        <div className="sales-card animate-fade">
          <PageSubHeader
            ...
            subTitle={
              i18n.catalog['enterpriseCore.nodes.viewManageAllOrganizationalUnitsStructure']
            }
            titleIcon="sitemap"
            actions={
              <Button variant="primary" icon="plus" onClick={openAddDialog}>
                {i18n.catalog['enterpriseCore.nodes.addUnit']}
              </Button>
            }
          />
          <Table
            columns={setupNodeColumns}
            data={nodes}
            keyExtractor={(item) => item.node_uuid}
            emptyMessage={i18n.catalog['enterpriseCore.nodes.noOrganizationalUnits']}
            isLoading={isLoading}
          />
        </div>
        <NodeFormDialog
    ...
    <>
      <div className="sales-card animate-fade">
        <PageSubHeader
          ...
          searchInput={
            <SearchableSelect
              ...
              options={nodes.map((n) => ({
                value: n.node_uuid,
                label: catalogText(i18n, 'common.general.notAvailable', {
                  value0: n.code,
                  value1: (n.attributes_json as any)?.name || '',
                }),
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                {selectedUuids.size > 0 && (
                  <Button variant="secondary" icon="edit" onClick={() => setBulkDialog(true)}>
                    {i18n.catalog['enterpriseCore.nodes.bulkEdit']}
                    {selectedUuids.size})
                  </Button>
                )}
                <Button variant="primary" icon="plus" onClick={openAddDialog}>
            ...
            nodes.length > 0 ? (
              ...
              >
                <Checkbox
                  ...
                  label={catalogText(i18n, 'enterpriseCore.nodes.selectAll', {
                    value0: nodes.length,
                  })}
                />
              </div>
            ...
            <>
              ...
              <Select
                ...
                options={[
                  ...
                  ...filteredMetaTypes.map((t) => ({
                    value: t.id,
                    label: t.display_name_ar || t.display_name,
                  })),
                ]}
              />
              <Select
              ...
              />
              <Button variant="secondary" onClick={loadNodes}>
                {i18n.catalog['common.general.search.alternative2']}
              </Button>
            </>
          }
        />
        ...
        <div style={{ marginTop: '0.5rem', fontSize: '0.75rem', color: 'var(--text-muted)' }}>
          {i18n.catalog['common.general.total']}
          {nodes.length} {i18n.catalog['common.general.unit']}
        </div>
      </div>
      ...
      <Dialog
        ...
        title={catalogText(i18n, 'enterpriseCore.nodes.bulkEditUnit', {
          value0: selectedUuids.size,
        })}
        footer={
          <>
            <Button variant="secondary" onClick={() => setBulkDialog(false)}>
              {i18n.catalog['common.general.cancel']}
            </Button>
            <Button variant="primary" onClick={handleBulkStatus}>
              {i18n.catalog['enterpriseCore.nodes.updateStatus']}
            </Button>
          </>
        }
      >
        <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '1rem' }}>
          {i18n.catalog['enterpriseCore.nodes.statusWillBeUpdated']}
          {selectedUuids.size}{' '}
          {i18n.catalog['enterpriseCore.nodes.unitSSimulatingMassChangeProcessSap']}
        </p>
        <Select
          label={i18n.catalog['enterpriseCore.nodes.newStatus']}
          value={bulkStatus}
      ...
      <ConfirmDialog
        ...
        message={i18n.catalog['enterpriseCore.nodes.areYouSureYouWantDeleteThisUnit']}
