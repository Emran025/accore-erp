...
export default function CashServicesPage() {
    ...
    const loadServices = async () => {
        try {
            const res = await fetchAPI(`${API_ENDPOINTS.COMMERCIAL.SERVICES.BASE}?per_page=200`);
    ...
    const loadInvoices = async () => {
        ...
        try {
            const res = await fetchAPI(`${API_ENDPOINTS.COMMERCIAL.SERVICES.SALES}?payment_type=cash&per_page=30`);
            const data = res.data ?? res;
    ...
    useEffect(() => { loadServices(); loadInvoices(); }, []);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            showToast("تم تسجيل فاتورة الخدمة النقدية بنجاح", "success");
            setLines([]);
            loadInvoices();
        } catch (e: any) {
    ...
    return (
        <MainLayout>
            <div className="sales-card compact">
                <PageSubHeader title="مبيعات الخدمات النقدية" titleIcon="banknote" />
                ...
                {lines.length > 0 && (
                    <div style={{ marginTop: "1rem", textAlign: "left", display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "0.25rem" }}>
                        ...
                        <Button variant="primary" icon="check" onClick={handleSave} disabled={saving} style={{ marginTop: "0.5rem" }}>
                            {saving ? "جارٍ الحفظ..." : "تسجيل الفاتورة النقدية"}
                        </Button>
                    </div>
                )}
            </div>
            ...
            <div className="sales-card compact" style={{ marginTop: "1.5rem" }}>
                <PageSubHeader title="الفواتير النقدية الأخيرة" titleIcon="receipt" />
                <Table
                    columns={invoiceColumns}
                    data={invoices}
                    loading={invoicesLoading}
                    emptyMessage="لا توجد فواتير نقدية"
                />
            </div>
            ...
            <Dialog isOpen={viewDialog} onClose={() => setViewDialog(false)} title="تفاصيل الفاتورة" size="sm">
                {viewedInvoice && (
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                        ...
                        <div>نوع الدفع: <strong>نقدي</strong></div>
