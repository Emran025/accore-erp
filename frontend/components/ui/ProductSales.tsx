...
export function ProductSales({ mode }: ProductSalesPageProps) {
    ...
    // Customers (only used in credit)
    const [customers, setCustomers] = useState<Customer[]>([]);
    const [selectedCustomer, setSelectedCustomer] = useState<Customer | null>(null);
    const [customerSearchTerm, setCustomerSearchTerm] = useState("");
    const [amountPaid, setAmountPaid] = useState("");

    // Sales Representative (optional in cash)
    ...
    const loadRepresentatives = useCallback(async () => {
        try {
            const response = await fetchAPI(API_ENDPOINTS.COMMERCIAL.SALES.REPRESENTATIVES.BASE + "?per_page=1000");
            if (response.success && response.data) {
    ...
    const loadInvoices = useCallback(async (page: number = 1) => {
        try {
            setIsLoading(true);
            const endpoint = `${API_ENDPOINTS.COMMERCIAL.SALES.INVOICES}?page=${page}&limit=${itemsPerPage}&payment_type=${mode}`;
            const response = await fetchAPI(endpoint);
            if (response.success && response.data) {
                setInvoices((response as unknown as PaginatedResponse<Invoice>).data || (response.data as Invoice[]));
    ...
    useEffect(() => {
        if (selectedProduct) {
            const cartItemEntries = invoiceItems.filter((item) => item.product_id === selectedProduct.id);
            const cartQtyInSubUnits = cartItemEntries.reduce((sum, item) => sum + (item.total_sub_units || 0), 0);
    ...
    }, [selectedProduct, invoiceItems, quantity, unitPrice]);
    ...
    const addItemToInvoice = () => {
        ...
        const costBasis = Number(selectedProduct.weighted_average_cost) || Number(selectedProduct.latest_purchase_price) || 0;
        ...
        try {
            const invoiceData: any = {
                ...
                items: invoiceItems.map((item) => ({
                    ...
                    quantity: isCash ? item.quantity : (item.total_sub_units || item.quantity),
            ...
            if (isCash) {
                invoiceData.sales_representative_id = selectedRepresentative?.id;
            } else {
            ...
            invoiceData.subtotal = invoiceData.items.reduce((sum: number, item: any) => sum + (item.unit_price * item.quantity), 0);
    ...
    const invoiceColumns: InvoiceTableColumn<Invoice>[] = [
        ...
        } as InvoiceTableColumn<Invoice>] : []),
        {
            ...
            dataLabel: "الإجراءات",
            render: (item) => (
                <ActionButtons actions={[{ icon: "view", title: "عرض", variant: "view", onClick: () => viewInvoice(item.id) }]} />
            ),
        },
    ];
    ...
    const currentInvoiceColumns: Column<InvoiceItem>[] = [
        ...
        { key: "quantity", header: "الكمية", dataLabel: "الكمية", render: (item) => `${item.quantity} ${item.unit_name}` },
        { key: "unit_price", header: "السعر", dataLabel: "السعر", render: (item) => formatCurrency(item.unit_price) },
        { key: "subtotal", header: "المجموع", dataLabel: "المجموع", render: (item) => formatCurrency(item.subtotal) },
        {
            key: "actions",
            header: "",
            dataLabel: "الإجراءات",
            render: (_, index) => (
                <ActionButtons actions={[{ icon: "trash", title: "حذف", variant: "delete", onClick: () => removeInvoiceItem(index) }]} />
            ),
        },
    ];
            ...
            <form id="invoice-form" onSubmit={(e) => { e.preventDefault(); addItemToInvoice(); }}>
                ...
                <div className="form-row">
                    ...
                    <div className="form-group">
                        ...
                        <input type="text" id="item-stock" value={itemStock} readOnly className="glass highlight-input" placeholder="0" />
                    </div>
                </div>
                <div className="form-row">
                    <div className="form-group">
                ...
                <div className="summary-stat-box">
                    ...
                    <button type="button" className="btn btn-primary btn-add" onClick={addItemToInvoice} data-icon="plus">إضافة {isCash ? 'للفاتورة' : ''}</button>
                </div>
            </form>
        </div>
    );
    ...
    const CustomerPanelForm = (
        <div className="sales-card compact animate-slide">
            ...
            <small style={{ color: "var(--text-light)", display: "block" }}>المبلغ الذي سيسدده العميل حالياً من قيمة الفاتورة</small>
        </div>
    );
                    ...
                    <div className={isCash ? "sales-card animate-slide" : "side-panel"} style={isCash ? { animationDelay: "0.1s" } : { display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                        ...
                        <div className="sales-card animate-slide" style={!isCash ? { animationDelay: "0.1s" } : {}}>
                            ...
                            <div className="current-invoice-table" style={{ width: '100%', overflowX: 'auto' }}>
                                <Table columns={currentInvoiceColumns} data={invoiceItems} keyExtractor={(_, index) => index} emptyMessage="لا توجد عناصر مضافة" />
                            </div>

                            <div className="invoice-adjustments">
                                <div className="discount-section">
                                    <div className="form-group" style={{ marginBottom: 0, flex: '1 1 200px', minWidth: '0' }}>
                                        <NumberInput id="invoice-discount" label="قيمة الخصم" value={discountValue} onChange={(val) => setDiscountValue(val)} min={0} placeholder="0.00" />
                                    </div>
                                    <SegmentedToggle label="نوع التخفيض" value={discountType} onChange={(val) => setDiscountType(val as "fixed" | "percent")} options={[{ value: "fixed", label: "مبلغ" }, { value: "percent", label: "نسبة" }]} />
                                    {isCash && (
                                        <div className="form-group" style={{ marginBottom: 0, flex: '1 1 250px', minWidth: '0' }}>
                                            <Label title="المندوب (اختياري)" />
                                            <SearchableSelect
                                                options={salesRepresentatives.map((r) => ({ value: r.id, label: r.name, subtitle: r.phone || "بدون هاتف" }))}
                            ...
                            <div className="form-row">
                                ...
                                <div className="summary-stat">
                                    ...
                                    <span className="stat-value">
                                        <span className={`badge ${isCash ? 'badge-success' : 'badge-warning'}`}>{isCash ? 'نقدي' : 'آجل (ذمم)'}</span>
                                    </span>
                                </div>
                            </div>
