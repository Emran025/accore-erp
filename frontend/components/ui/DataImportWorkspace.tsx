...
const STEP_LABELS = ["Source", "Link fields", "Review", "Complete"];
...
function parseBoolean(value: unknown, fallback: boolean): boolean {
    ...
    if (["false", "0", "no", "n", "لا", "غير متاح"].includes(normalized)) return false;
    return fallback;
}
...
function parseWorkbook(file: File): Promise<ParsedSource> {
    return file.arrayBuffer().then((buffer) => {
        ...
        if (!headers.length || !rows.length) throw new Error("The selected file has no header row or data records.");
...
function normalizeRow(source: Record<string, string>, mapping: Record<string, string>, fields: ImportField[]): ImportRow {
    ...
    raw.item_type = itemType === "service" || itemType === "خدمة" ? "service" : itemType === "raw_material" || itemType === "raw material" || itemType === "مادة خام" ? "raw_material" : "product";
...
function validateRow(row: ImportRow, fields: ImportField[]): string[] {
    ...
    fields.filter((field) => field.required && (!field.dependsOn || row[field.dependsOn] !== "service")).forEach((field) => {
        if (textValue(row[field.id]) === "") errors.push(`${field.label} is required`);
    });
    ["unit_price", "purchase_price", "stock_quantity", "items_per_unit", "low_stock_threshold", "minimum_profit_margin"].forEach((key) => {
        if (key in row && (!Number.isFinite(Number(row[key])) || Number(row[key]) < 0)) errors.push(`${key} must be a non-negative number`);
    });
    if (!["product", "service", "raw_material"].includes(textValue(row.item_type))) errors.push("Item class is not supported");
    if (Number(row.items_per_unit) < 1) errors.push("Items per unit must be at least 1");
    return errors;
}

const FIELD_GROUPS: Array<{ key: ImportField["group"]; label: string }> = [
    { key: "identity", label: "Identity" },
    { key: "classification", label: "Class first" },
...
export function DataImportWorkspace({ title, subtitle, fields, onImport, onClose, maxRows = 1000 }: DataImportWorkspaceProps) {
    ...
    const readFile = async (file?: File) => {
        ...
        try {
            ...
            showToast(error instanceof Error ? error.message : "Unable to read the selected file.", "error");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const goToReview = () => {
        ...
        if (errors.some((item) => item.length)) {
            showToast("Resolve the highlighted validation issues before importing.", "error");
            return;
        }
        setActiveStep(2);
    };
    ...
    const commitImport = async () => {
        ...
        if (!rows.length || errors.some((item) => item.length)) {
            showToast("The import contains unresolved records.", "error");
            return;
        }
        try {
            setIsLoading(true);
            ...
            showToast(error instanceof Error ? error.message : "The import could not be completed.", "error");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <section className="data-import-workspace" aria-label={title}>
            <header className="data-import-header">
                <div>
                    <span className="data-import-eyebrow">Reusable data bridge</span>
                    <h2>{title}</h2>
                    <p>{subtitle}</p>
                </div>
                <div className="data-import-header-actions">
                    <span className="data-import-format-note">Excel · CSV · TSV</span>
                    <Button variant="secondary" onClick={onClose}>Close</Button>
                </div>
            </header>

            <div className="data-import-steps" role="list" aria-label="Import progress">
            ...
            {activeStep === 0 && (
                <div className="data-import-source-panel">
                    <div className={`data-import-dropzone ${isDragging ? "is-dragging" : ""}`} onDragOver={(event) => { event.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} onDrop={(event) => { event.preventDefault(); setIsDragging(false); void readFile(event.dataTransfer.files[0]); }}>
                        ...
                        <h3>Drop an external data file here</h3>
                        <p>We will recognize the source columns, suggest compatible internal fields, and keep the original values available for review.</p>
                        <Button variant="primary" onClick={() => inputRef.current?.click()} disabled={isLoading}>{isLoading ? "Reading file…" : "Choose Excel or CSV file"}</Button>
                    ...
                    <div className="data-import-source-guidance">
                        <div><strong>External model</strong><span>Supplier headers, legacy exports, or ad-hoc spreadsheets.</span></div>
                        <div><strong>Internal model</strong><span>Canonical item fields used by products, inventory, and reports.</span></div>
                        <div><strong>Safe handoff</strong><span>Validation runs before any record is written.</span></div>
                    </div>
                </div>
            )}
            ...
            {activeStep === 1 && source && (
                <div className="data-import-mapping-panel">
                    <aside className="data-import-source-summary">
                        <span className="data-import-eyebrow">Detected source</span>
                        <strong>{source.fileName}</strong>
                        <span>{source.rows.length} records · {source.headers.length} columns</span>
                        <div className="data-import-header-chips">{source.headers.map((header) => <span key={header}>{header}</span>)}</div>
                        <button type="button" className="data-import-link-button" onClick={() => setActiveStep(0)}>Choose another file</button>
                    </aside>
                    <div className="data-import-field-linker">
                        <div className="data-import-section-heading"><div><span className="data-import-eyebrow">External → internal</span><h3>Link the source class to the item model</h3></div><span className="data-import-match-count">{Object.keys(mapping).length}/{fields.length} suggested</span></div>
                        <p className="data-import-helper">Start with the item class. Dependent inventory and selling fields are intentionally grouped below so one field can explain the others instead of forcing duplicate input.</p>
                        {FIELD_GROUPS.map((group) => {
                            ...
                            return <div className={`data-import-field-group ${group.key === "classification" ? "priority" : ""}`} key={group.key}>
                                <div className="data-import-group-label">{group.label}{group.key === "classification" && <span>Unlocks dependent fields</span>}</div>
                                {groupFields.map((field) => <div className="data-import-map-row" key={field.id}>
                                    ...
                                    <Select value={mapping[field.id] || "__unmapped__"} onChange={(event) => remapRows({ ...mapping, [field.id]: event.target.value === "__unmapped__" ? "" : event.target.value })} options={[{ value: "__unmapped__", label: "Leave unmapped" }, ...source.headers.map((header) => ({ value: header, label: header }))]} />
                                </div>)}
                            </div>;
                        })}
                        <div className="data-import-actions"><Button variant="secondary" onClick={() => setActiveStep(0)}>Back</Button><Button variant="primary" onClick={goToReview}>Review {rows.length} records</Button></div>
                    </div>
                </div>
            )}
            ...
            {activeStep === 2 && source && (
                <div className="data-import-review-panel">
                    <div className="data-import-review-summary"><div><span className="data-import-eyebrow">Integrity checkpoint</span><h3>Review normalized items before the final import</h3><p>Product cards show the interpreted class and commercial values. The editable raw grid keeps every mapped value available for correction.</p></div><div className="data-import-class-summary">{Object.entries(classSummary).map(([key, value]) => <span key={key}><strong>{value}</strong>{key.replace("_", " ")}</span>)}</div></div>
                    <div className="data-import-gallery">{rows.slice(0, 6).map((row, index) => <article className={`data-import-product-card ${rowErrors[index]?.length ? "has-errors" : ""}`} key={`${textValue(row.name)}-${index}`}><div className="data-import-card-topline"><span>{textValue(row.item_type).replace("_", " ")}</span><small>Row {index + 2}</small></div><h4>{textValue(row.name) || "Unnamed item"}</h4><div className="data-import-card-meta"><span>{textValue(row.unit_name) || "piece"}</span><span>{Number(row.stock_quantity || 0)} in stock</span></div><strong>{Number(row.unit_price || 0).toFixed(2)}</strong>{rowErrors[index]?.length ? <div className="data-import-error-text">{rowErrors[index].join(" · ")}</div> : <div className="data-import-success-text">Ready for import</div>}</article>)}</div>
                    <div className="data-import-raw-heading"><div><span className="data-import-eyebrow">Raw values / editable</span><h3>Correct empty or misread fields in place</h3></div><span>{rows.length > 6 ? `Showing first 6 of ${rows.length}` : `${rows.length} records`}</span></div>
                    <div className="data-import-raw-table-wrap"><table className="data-import-raw-table"><thead><tr><th>Row</th>{visibleFields.map((field) => <th key={field.id}>{field.label}</th>)}</tr></thead><tbody>{rows.slice(0, 6).map((row, rowIndex) => <tr key={rowIndex}><td>{rowIndex + 2}</td>{visibleFields.map((field) => <td key={field.id}>{field.type === "boolean" ? <select value={row[field.id] ? "true" : "false"} onChange={(event) => updateRow(rowIndex, field, event.target.value)}><option value="true">Yes</option><option value="false">No</option></select> : <input value={textValue(row[field.id])} type={field.type === "number" ? "number" : "text"} onChange={(event) => updateRow(rowIndex, field, event.target.value)} />}</td>)}</tr>)}</tbody></table></div>
                    <div className="data-import-actions"><Button variant="secondary" onClick={() => setActiveStep(1)}>Back to mapping</Button><Button variant="primary" onClick={() => void commitImport()} disabled={isLoading}>{isLoading ? "Importing…" : `Import ${rows.length} items`}</Button></div>
                </div>
            )}

            {activeStep === 3 && result && (
                <div className="data-import-complete"><div className="data-import-complete-mark">✓</div><span className="data-import-eyebrow">Import complete</span><h3>{result.imported} records are now available in the internal catalogue</h3><p>{result.message || "The imported products can now be browsed from the product gallery and used by inventory reports."}</p><Button variant="primary" onClick={onClose}>Return to products</Button></div>
            )}
        </section>
    );
}
